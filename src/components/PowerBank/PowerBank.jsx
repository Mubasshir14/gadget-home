...
const PowerBank = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:5000/product')
            .then((response) => {
                const newArrivalProducts = response.data.filter(product => product.category === 'powerbank');
    ...
    return (
        <div>
            <Helmet>
                <title>Gadget Home || Power Bank</title>
            </Helmet>
            <div className='max-w-screen-xl mx-auto md:mb-20 p-2'>
                <div className="flex w-full flex-col">
                    <div className=" text-[#59C6D2] font-bold text-xl md:text-2xl font-cinzel text-center mb-8">Power Bank</div>
                </div>
